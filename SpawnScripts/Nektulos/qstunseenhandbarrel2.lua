...
function InRange(NPC, Spawn)
    if GetQuestStep(Spawn, ProveYourself) == 1 then
...
function casted_on(NPC, Spawn, SpellName)
		if SpellName == 'Leave the supplies' then
            if not QuestStepIsComplete(Spawn, ProveYourself, 1) then
                zone = GetZone(Spawn)
                SpawnByLocationID(zone, 100434)
                SetStepComplete(Spawn, ProveYourself, 1)
