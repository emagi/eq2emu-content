--[[
    Script Name    : SpawnScripts/ThunderingSteppes/GnollCampControllerTest.lua
    Script Author  : LordPazuzu
    Script Date    : 2024.07.23 11:07:47
    Script Purpose : 
                   : 
--]]
...
function spawn(NPC, Spawn)
    ...
    ControlObject = 133793322   --Loc ID for camp controller.  Camp controller is an invisible cube spawn containing this script.
    ...
    SubBossGroup = 1057717    --GroupID for SubBoss Group
    BossGroup = 1057718       --GroupID for Boss Group
    PropGroup = 1057708       --GroupID for camp props i.e. tents, campfires, etc
    BuffObject = 133793352      --LocID for buff object to spawn in place of named.  i.e. gnollish ark 
    
    Group1(NPC, Spawn, 1057709, 1057710) --Enter in group IDs for variant groups in that spawn.  Default script is setup for 2 variants, more can be added.
    Group2(NPC, Spawn, 1057711, 1057712)
    Group3(NPC, Spawn, 1057713, 1057714)
    Group4(NPC, Spawn, 1057715, 1057716)
...
function SubBossCheck(NPC, Spawn)
    local zone = GetZone(NPC)
    if  GetSpawnByGroupID(zone, G1) ~=nil then
        AddTimer(NPC, 6000, "SubBossCheck", 1, Spawn)
    elseif
        GetSpawnByGroupID(zone, G2) ~=nil then
        AddTimer(NPC, 6000, "SubBossCheck", 1, Spawn)
    elseif
        GetSpawnByGroupID(zone, G3) ~=nil then
        AddTimer(NPC, 6000, "SubBossCheck", 1, Spawn)
    elseif
        GetSpawnByGroupID(zone, G4) ~=nil then
        AddTimer(NPC, 6000, "SubBossCheck", 1, Spawn)
    else 
