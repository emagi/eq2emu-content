--[[
	Script Name	: SpawnScripts/OutpostOverlord/TrainerJaylaSurfrider.lua
	Script Purpose	: Trainer Jayla Surfrider <Know Your Fate>
...
function hailed(NPC, Spawn)
	...
	PlayFlavor(NPC, "voiceover/english/voice_emotes/greetings/greetings_" .. choice .. "_1021.mp3", "", "", 0, 0, Spawn)

	if HasQuest(Spawn, 492) and GetQuestStep(Spawn, 492) == 2 then
		AddConversationOption(conversation, "Do you know a man called Charles Arker?", "dlg_37_1")
	end

	AddConversationOption(conversation, "What can cause my death? ", "dlg_3_1")
...
function dlg_3_1(NPC, Spawn)
	...
	StartConversation(conversation, NPC, Spawn, "Death can come in many ways: being bested on the field of battle, falling from great heights, drowning in water, falling into lava, standing too close to an exploding chest, and so on. It's a dangerous world out there, XXXXX.")
end
...
function dlg_3_9(NPC, Spawn)
	...
	StartConversation(conversation, NPC, Spawn, "Your equipped items usually take about 10% wear with each death. When they are down to zero durability, you no longer receive any benefit from them. Don't let your equipment ever get into such poor condition, XXXXX!")
end
...
-- Charles Arker quest from Field Investigator
function dlg_37_1(NPC, Spawn)
	SetStepComplete(Spawn, 492, 2)
	FaceTarget(NPC, Spawn)
	conversation = CreateConversation()

	AddConversationOption(conversation, "Not yet. Thank you, though.")
	StartConversation(conversation, NPC, Spawn, "Sorry, never heard of him. Did he die recently?")
end
