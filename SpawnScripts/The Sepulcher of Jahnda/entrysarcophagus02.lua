--[[
    Script Name    : SpawnScripts/The Sepulcher of Jahnda/entrysarcophagus02.lua
    Script Author  : Premierio015
    Script Date    : 2021.10.22 07:10:36
    Script Purpose : 
                   : 
--]]
...
if SpellName == 'Whisper a prayer' then
    ...
    SetTempVariable(Spawn, "sarcophagus2_used", "true")
...
elseif SpellName == 'Defile remains' then
     ...
     local object_remains = GetSpawnByLocationID(zone, 133772675) 
     local check_spawn = GetSpawnByLocationID(zone, 133772681)
     if check_spawn == nil then
     local a_desecrated_remains = SpawnByLocationID(zone, 133772681) -- a_desecrated_remains02 ID
     Despawn(object_remains)
     AddTimer(NPC, 4600, "visual_effect", 1)
     end
     SpawnSet(NPC, "show_command_icon", 0)
    SpawnSet(NPC, "display_hand_icon", 0)
     SetTempVariable(Spawn, "remains_defiled_2", "true")
...
function visual_effect(NPC, Spawn)
  local zone = GetZone(NPC)
   local a_desecrated_remains = GetSpawnByLocationID(zone, 133772681) -- a_desecrated_remains02 ID
