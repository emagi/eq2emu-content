--[[
    Script Name    : SpawnScripts/The Sepulcher of Jahnda/entrysarcophagus05.lua
    Script Author  : Premierio015
    Script Date    : 2021.10.22 07:10:22
    Script Purpose : 
                   : 
--]]
...
function casted_on(NPC, Spawn, SpellName)
    ...
    SetTempVariable(Spawn, "sarcophagus5_used", "true")
...
elseif SpellName == 'Defile remains' then
     ...
     local object_remains = GetSpawnByLocationID(zone, 133772678) 
     local check_spawn = GetSpawnByLocationID(zone, 133772684)
     if check_spawn == nil then
     local a_desecrated_remains = SpawnByLocationID(zone, 133772684) -- a_desecrated_remains05 ID
     Despawn(object_remains)
      AddTimer(NPC, 4600, "visual_effect", 1)
     end
     SpawnSet(NPC, "show_command_icon", 0)
    SpawnSet(NPC, "display_hand_icon", 0)
    SetTempVariable(Spawn, "remains_defiled_5", "true")
...
function visual_effect(NPC, Spawn)
  local zone = GetZone(NPC)
  local a_desecrated_remains = GetSpawnByLocationID(zone, 133772684) -- a_desecrated_remains05 ID  
