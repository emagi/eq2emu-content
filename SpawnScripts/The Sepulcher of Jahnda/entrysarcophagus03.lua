--[[
    Script Name    : SpawnScripts/The Sepulcher of Jahnda/entrysarcophagus03.lua
...
if SpellName == 'Whisper a prayer' then
    ...
    SetTempVariable(Spawn, "sarcophagus3_used", "true")
...
elseif SpellName == 'Defile remains' then
     ...
     local object_remains = GetSpawnByLocationID(zone, 133772676)
     local check_spawn = GetSpawnByLocationID(zone, 133772682)
     if check_spawn == nil then
     local a_desecrated_remains = SpawnByLocationID(zone, 133772682) -- a_desecrated_remains03 ID
     Despawn(object_remains)
      AddTimer(NPC, 4600, "visual_effect", 1)
     end
     SpawnSet(NPC, "show_command_icon", 0)
    SpawnSet(NPC, "display_hand_icon", 0)	 
    SetTempVariable(Spawn, "remains_defiled_3", "true")
...
function visual_effect(NPC, Spawn)
  local zone = GetZone(NPC)
     local a_desecrated_remains = GetSpawnByLocationID(zone, 133772682) -- a_desecrated_remains03 ID
